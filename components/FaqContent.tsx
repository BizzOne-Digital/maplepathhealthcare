          ...
          <div className="space-y-3" itemScope itemType="https://schema.org/FAQPage">
            {faqs.map((f, i) => (
              <AnimateIn key={i} delay={i * 40}>
                <div className={`bg-white border-[1.5px] rounded-xl overflow-hidden transition-all ${open === i ? 'border-[#5DA6DD] shadow-md faq-open' : 'border-[rgba(42,67,115,.1)]'}`} itemScope itemProp="mainEntity" itemType="https://schema.org/Question">
                  <button onClick={() => setOpen(open === i ? null : i)} className="w-full flex justify-between items-center px-6 py-5 text-left gap-4" aria-expanded={open === i}>
                    <h3 className="text-[15px] font-semibold text-[#1C3162] leading-snug" itemProp="name">{f.q}</h3>
                  ...
                  <div className="faq-answer px-6 border-t border-[rgba(42,67,115,.08)]" itemScope itemProp="acceptedAnswer" itemType="https://schema.org/Answer">
                    <p className="text-[14px] text-[#5C6B80] leading-relaxed pt-4 pb-5" itemProp="text">{f.a}</p>
                  </div>
                </div>
              </AnimateIn>
            ))}
